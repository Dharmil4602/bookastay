...
export const addReview = async (req, res) => {
    try {
        const { listingId, userId, firstName, lastName, rating, comment } = req.body;
        var newReview;
        // Find the listing by ID
        const listing = await Listing.findById(listingId);

        // Check if the user already has a review
        const existingReviewIndex = listing.reviews.findIndex(review => String(review.user) === userId);
        ...
        const totalRatings = listing.reviews.reduce((total, review) => total + review.rating, 0);
        ...
        res.status(201).json({ message: 'Review added successfully', newReview: listing.reviews[existingReviewIndex] || newReview });
    } catch (error) {
...
export const deleteReview = async (req, res) => {
    try {
        ...
        const averageRating = numberOfReviews > 0 ? totalRatings / numberOfReviews : 0;
